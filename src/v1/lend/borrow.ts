...
async function getTokenPairInfo(indexerClient: Indexer, tokenPair: TokenPair): Promise<TokenPairInfo> {
  ...
  const tb = BigInt(getParsedValueFromState(state, 'total_borrowed') || 0);
  const tbl = BigInt(getParsedValueFromState(state, 'total_borrowed_limit') || 18446744073709551615);
